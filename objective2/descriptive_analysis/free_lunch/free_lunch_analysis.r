...
calculate_statistics <- function(score_column) {
  ...
  size_muestra <- format(nrow(muestra_free_lunch), nsmall = 0)
...
free_lunch_stats <- calculate_statistics(muestra_free_lunch$promedio)

column_names <- c("Tamaño de Muestra", "Media", "Mediana","Moda","Rango",  "Varianza",  "Desviación Estándar")

free_lunch_table <- make_table(column_names, free_lunch_stats) %>% change_table_names()

make_table_plot(free_lunch_table,"objective2/descriptive_analysis/free_lunch/images/free_lunch_table_plot.png")

make_histogram_plot(
  data = muestra_free_lunch,
  column = "promedio",
  binwidth = 5,
  fill_color = "blue",
  border_color = "black",
  ...
  x_label = "Promedio",
  file_path = "objective2/descriptive_analysis/free_lunch/images/free_lunch_hist_plot.png"
)
...
make_box_plot(
  ...
  y_label = "Promedio",
  file_path = "objective2/descriptive_analysis/free_lunch/images/free_lunch_box_plot.png",
  outlier_color = "red",
  width = 4,
  height = 6  
)