...
calculate_statistics <- function(score_column) {
  ...
  size_muestra <- format(nrow(muestra_sin_universidad), nsmall = 0)
...
not_university_stats <- calculate_statistics(muestra_sin_universidad$promedio)

column_names <- c("Tamaño de Muestra", "Media", "Mediana","Moda","Rango",  "Varianza",  "Desviación Estándar")

not_university_table <- make_table(column_names, not_university_stats) %>% change_table_names()

make_table_plot(not_university_table,"objective1/descriptive_analysis/not_university/images/not_university_table_plot.png")

make_histogram_plot(
  data = muestra_sin_universidad,
  column = "promedio",
  binwidth = 5,
  fill_color = "blue",
  border_color = "black",
  title = "",
  x_label = "Promedio",
  file_path = "objective1/descriptive_analysis/not_university/images/not_university_hist_plot.png"
)
...
make_box_plot(
  ...
  title = "",
  y_label = "Promedio",
  file_path = "objective1/descriptive_analysis/not_university/images/not_university_box_plot.png",
  outlier_color = "red",
  width = 4,
  height = 6  
)